...
//  Disable  all intterrupts
  ...
  static QF_MPOOL_EL(QEvt) smlPoolSto[10U]; // small pool
  QF_poolInit(smlPoolSto, sizeof(smlPoolSto), sizeof(smlPoolSto[0]));


  static QEvt const *serverQueueSto[SERVER_QUEUE_EN];
  static StackType_t serverStack[SERVER_TASK_STACK_SIZE];

  Server_ctor(); // instantiate the Server active object

  QActive_setAttr(AO_Server, TASK_NAME_ATTR, "Server");
  QACTIVE_START(AO_Server, // AO to start
  SERVER_TASK_PRIORITY, // QP priority of the AO
  serverQueueSto, // event queue storage
  Q_DIM(serverQueueSto), // queue length [events]
  serverStack, // stack storage
  sizeof(serverStack), // stack size [bytes]
  (QEvt *)0); // initialization param (not used)
